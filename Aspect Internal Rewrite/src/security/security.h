...
public:
        ...
        std::string iv)
    {
        ...
        CryptoPP::CBC_Mode<CryptoPP::AES>::Encryption encryption(
            (byte*)key.c_str(), key.length(), (byte*)iv.c_str());
        CryptoPP::StringSource encryptor(
            str_in, true,
            new CryptoPP::StreamTransformationFilter(
                encryption, new CryptoPP::Base64Encoder(new CryptoPP::StringSink(str_out), false)));
        return str_out;
    }
    ...
    inline bool authenticate(char* username, char* password)
    {
        ...
        std::string iv64;
        std::string key264;
        CryptoPP::StringSource dummy0001(
            iv, true, new CryptoPP::Base64Encoder(new CryptoPP::StringSink(iv64)));
        CryptoPP::StringSource dummy0002(
            key_2, true,
        ...
        CryptoPP::StringSource dummy0003(
            combo, true,
            new CryptoPP::Base64Encoder(new CryptoPP::StringSink(response)));
        CryptoPP::StringSource dummy0004(
            response, true,
            new CryptoPP::Base64Decoder(new CryptoPP::StringSink(responseundone)));
        CryptoPP::StringSource dummy0005(
            response64, true,
            new CryptoPP::Base64Decoder(
                new CryptoPP::StringSink(response64undone)));
