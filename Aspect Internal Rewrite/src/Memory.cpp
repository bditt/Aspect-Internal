...
namespace
{
	...
	bool Compare(const std::vector<unsigned char>& bytes, const std::string& mask, const std::uintptr_t start)
	{
...
namespace Memory
{
	...
	std::uintptr_t Scan(const std::uintptr_t start, std::uintptr_t end, const std::string& pattern, const int flags, const int offset)
	{
	...
	std::uintptr_t Scan(const std::string& pattern, const int flags, const int offset)
	{
		MODULEINFO mi;
		GetModuleInformation(GetCurrentProcess(), GetModuleHandle(nullptr), &mi, sizeof(MODULEINFO));
		return Scan(reinterpret_cast<std::uintptr_t>(mi.lpBaseOfDll), reinterpret_cast<std::uintptr_t>(mi.lpBaseOfDll) + mi.SizeOfImage, pattern, flags, offset);
	}
}