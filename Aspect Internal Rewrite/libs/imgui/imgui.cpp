...
static const float WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER = 0.04f; // Reduce visual noise by only highlighting the border after a
    // certain time.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.CursorPosPrevLine.y = window->DC.CursorPos.y - line_height; // Setting those fields so that SetScrollHereY() can
        // properly function after the end of our clipper usage.
    window->DC.PrevLineSize.y = (line_height - GImGui->Style.ItemSpacing.y); // If we end up needing more accurate data (to e.g. use
    ...
    if (window->DC.ColumnsSet)
        window->DC.ColumnsSet->LineMinY = window->DC.CursorPos
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 0) // Step 0: the clipper let you process the first element, regardless
    ...
    bool tab_stop)
{
    ...
    if (tab_stop && (g.ActiveId == id) && window->FocusIdxAllRequestNext == INT_MAX && window->FocusIdxTabRequestNext == INT_MAX && !g.IO.KeyCtrl && IsKeyPressedMap(ImGuiKey_Tab))
        window->FocusIdxTabRequestNext = window->FocusIdxTabCounter + (g.IO.KeyShift ? (is_tab_stop ? -1 : 0) : +1); // Modulo on index will be applied at the end of frame once
...
void ImGui::NewFrame()
{
    ...
    IM_ASSERT(g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f && "Invalid style setting. Alpha cannot be negative (allows us to "
                                                                "avoid a few clamps in color computations)");
    IM_ASSERT((g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount) && "Forgot to call Render() or EndFrame() at the end of the previous "
                                                                          "frame?");
    for (int n = 0; n < ImGuiKey_COUNT; n++)
        IM_ASSERT(g.IO.KeyMap[n] >= -1 && g.IO.KeyMap[n] < IM_ARRAYSIZE(g.IO.KeysDown) && "io.KeyMap[] contains an out of bound value (need to be 0..512, "
    ...
    if (g.SettingsDirtyTimer > 0.0f) {
        ...
        if (g.SettingsDirtyTimer <= 0.0f) {
            if (g.IO.IniFilename != NULL)
                ...
                g.IO.WantSaveIniSettings = true; // Let user know they can call SaveIniSettingsToMemory().
...
static void AddWindowToDrawData(ImVector<ImDrawList*>* out_renderList,
    ImGuiWindow* window)
{
    ImGuiContext& g = *GImGui;
    g.IO.MetricsRenderWindows++;
    AddDrawListToDrawData(out_renderList, window->DrawList);
    for (int i = 0; i < window->DC.ChildWindows.Size; i++) {
        ImGuiWindow* child = window->DC.ChildWindows[i];
        if (IsWindowActiveAndVisible(
                child)) // clipped children may have been marked not active
            AddWindowToDrawData(out_renderList, child);
    }
}
...
void ImGui::EndFrame()
{
    ...
    if (g.CurrentWindowStack.Size != 1) {
        if (g.CurrentWindowStack.Size > 1) {
            IM_ASSERT(g.CurrentWindowStack.Size == 1 && "Mismatched Begin/BeginChild vs End/EndChild calls: did you "
...
void ImGui::Render()
{
    ...
    windows_to_render_front_most[0] = (g.NavWindowingTarget && !(g.NavWindowingTarget->Flags & ImGuiWindowFlags_NoBringToFrontOnFocus))
    ...
    for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++)
        if (windows_to_render_front_most[n] && IsWindowActiveAndVisible(windows_to_render_front_most[n])) // NavWindowingTarget is always
...
static ImVec2 CalcSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
    ...
    if (!(window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_AlwaysAutoResize))) {
        ...
        new_size.y = ImMax(
            new_size.y,
            window->TitleBarHeight() + window->MenuBarHeight() + ImMax(0.0f, g.Style.WindowRounding - 1.0f)); // Reduce artifacts with very small windows
    }
    return new_size;
}
    ...
    ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count;
        ...
        if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
            ...
            ImVec2 corner_target = g.IO.MousePos - g.ActiveIdClickOffset + ImLerp(grip.InnerDir * grip_hover_outer_size, grip.InnerDir * -grip_hover_inner_size,
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    bool window_just_activated_by_user = (window->LastFrameActive < current_frame - 1); // Not using !WasActive because the implicit "Debug"
    ...
    if (first_begin_of_the_frame) {
        ...
        if (!(flags & ImGuiWindowFlags_ChildWindow)) {
            if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && g.IO.DisplaySize.x > 0.0f && g.IO.DisplaySize.y > 0.0f) // Ignore zero-sized display explicitly to avoid losing
            ...
            if (!(flags & ImGuiWindowFlags_NoResize)) {
                for (int resize_grip_n = 0; resize_grip_n < resize_grip_count;
                    ...
                    window->DrawList->PathLineTo(
                        corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(window_border_size, grip_draw_size) : ImVec2(grip_draw_size, window_border_size)));
                    window->DrawList->PathLineTo(
                        corner + grip.InnerDir * ((resize_grip_n & 1) ? ImVec2(grip_draw_size, window_border_size) : ImVec2(window_border_size, grip_draw_size)));
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
            ...
            if (flags & ImGuiWindowFlags_UnsavedDocument) {
                ImVec2 marker_pos = ImVec2(ImMax(text_r.Min.x,
                                               text_r.Min.x + (text_r.GetWidth() - text_size.x) * style.WindowTitleAlign.x)
                                            + text_size.x,
                                        text_r.Min.y)
...
void ImGui::SetCurrentFont(ImFont* font)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(
        font && font->IsLoaded()); // Font Atlas not created. Did you call
    ...
    ImGuiCond cond)
{
    ...
    window->DC.CursorMaxPos += (window->Pos - old_pos); // And more importantly we need to adjust this
...
{
    ImGuiContext& g = *GImGui;
    g.NextWindowData.ContentSizeVal = size; // In Begin() we will add the size of window decorations (title
...
{
    ImGuiWindow* window = GetCurrentWindow();
    window->ScrollTarget.y = scroll_y + window->TitleBarHeight() + window->MenuBarHeight(); // title bar height canceled out when using
        // ScrollTargetRelY
...
void ImGui::ClosePopupToLevel(int remaining, bool apply_focus_to_window_under)
{
    ...
    // manifested until now because UpdateMouseMovingWindow() would call
...
void ImGui::CloseCurrentPopup()
{
    ...
    if (popup_idx < 0 || popup_idx >= g.OpenPopupStack.Size || g.BeginPopupStack[popup_idx].PopupId != g.OpenPopupStack[popup_idx].PopupId)
        return;
    while (
        popup_idx > 0 && g.OpenPopupStack[popup_idx].Window && (g.OpenPopupStack[popup_idx].Window->Flags & ImGuiWindowFlags_ChildMenu))
        popup_idx--;
...
void ImGui::EndPopup()
{
    ...
    (void)g;
    IM_ASSERT(
        g.CurrentWindow->Flags & ImGuiWindowFlags_Popup); // Mismatched BeginPopup()/EndPopup() calls
    ...
    ImGuiPopupPositionPolicy policy)
{
    ...
    if (policy == ImGuiPopupPositionPolicy_ComboBox) {
        const ImGuiDir dir_prefered_order[ImGuiDir_COUNT] = {
            ImGuiDir_Down, ImGuiDir_Right, ImGuiDirLeft, ImGuiDir_Up
        };
    ...
    const ImGuiDir dir_prefered_order[ImGuiDir_COUNT] = {
        ImGuiDir_Right, ImGuiDir_Down, ImGuiDir_Up, ImGuiDirLeft
    };
    for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++) {
        const ImGuiDir dir = (n == -1) ? *last_dir : dir_prefered_order[n];
        if (n != -1 && dir == *last_dir) // Already tried this direction?
            continue;
        float avail_w = (dir == ImGuiDirLeft ? r_avoid.Min.x : r_outer.Max.x) - (dir == ImGuiDir_Right ? r_avoid.Max.x : r_outer.Min.x);
        float avail_h = (dir == ImGuiDir_Up ? r_avoid.Min.y : r_outer.Max.y) - (dir == ImGuiDir_Down ? r_avoid.Max.y : r_outer.Min.y);
        if (avail_w < size.x || avail_h < size.y)
            continue;
        ImVec2 pos;
        pos.x = (dir == ImGuiDirLeft)
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu) {
        ...
        float horizontal_overlap = g.Style.ItemSpacing
...
ImGuiDir ImGetDirQuadrantFromDelta(float dx, float dy)
{
    if (ImFabs(dx) > ImFabs(dy))
        return (dx > 0.0f) ? ImGuiDir_Right : ImGuiDirLeft;
...
{
    if (move_dir == ImGuiDirLeft || move_dir == ImGuiDir_Right) {
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    const ImRect& curr = g.NavScoringRectScreen; // Current modified source rect (NB: we've
    ...
    if (dbx != 0.0f || dby != 0.0f) {
        ...
        quadrant = (window->DC.LastItemId < g.NavId) ? ImGuiDirLeft : ImGuiDir_Right;
    }
    ...
    if (result->DistBox == FLT_MAX && dist_axial < result->DistAxial) // Check axial match
        if (g.NavLayer == 1 && !(g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
            if ((g.NavMoveDir == ImGuiDirLeft && dax < 0.0f) || (g.NavMoveDir == ImGuiDir_Right && dax > 0.0f) || (g.NavMoveDir == ImGuiDir_Up && day < 0.0f) || (g.NavMoveDir == ImGuiDir_Down && day > 0.0f)) {
    ...
    ImGuiNavMoveFlags move_flags)
{
    ...
    if (g.NavMoveDir == ImGuiDirLeft && (move_flags & (ImGuiNavMoveFlags_WrapX | ImGuiNavMoveFlags_LoopX))) {
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
    ...
    if (t < 0.0f && mode == ImGuiInputReadMode_Released) // Return 1.0f when just released,
...
#undef NAV_MAP_KEY
    ...
    // FIXME: g.NavInitRequest is always false at this point, investigate the
    // intent of operation done here.
    ...
    if (g.NavMoveRequestForward == ImGuiNavForward_None) {
        ...
        if (g.NavWindow && !g.NavWindowingTarget && allowed_dir_flags && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs)) {
            if ((allowed_dir_flags & (1 << ImGuiDirLeft)) && IsNavInputPressedAnyOfTwo(ImGuiNavInput_DpadLeft, ImGuiNavInput_KeyLeft_, ImGuiInputReadMode_Repeat))
                g.NavMoveDir = ImGuiDirLeft;
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget) {
        ...
        const float scroll_speed = ImFloor(window->CalcFontSize() * 100 * g.IO.DeltaTime + 0.5f); // We need round the scrolling speed because sub-pixel
            // scroll isn't reliably supported.
        if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll && g.NavMoveRequest) {
            if (g.NavMoveDir == ImGuiDirLeft || g.NavMoveDir == ImGuiDir_Right)
                SetWindowScrollX(
                    window, ImFloor(window->Scroll.x + ((g.NavMoveDir == ImGuiDirLeft) ? -1.0f : +1.0f) * scroll_speed));
            if (g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down)
                SetWindowScrollY(
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (g.NavWindowingTarget && g.NavInputSource == ImGuiInputSource_NavGamepad) {
        ...
        const int focus_change_dir = (int)IsNavInputPressed(ImGuiNavInput_FocusPrev,
        ...
        if (!IsNavInputDown(ImGuiNavInput_Menu)) {
            g.NavWindowingToggleLayer &= (g.NavWindowingHighlightAlpha < 1.0f); // Once button was held long enough we don't consider it a
    ...
    ImGuiDragDropFlags flags)
{
    ...
    payload.Delivery = was_accepted_previously && !IsMouseDown(g.DragDropMouseButton); // For extern drag sources affecting
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs {
        ...
        static void NodeTabBar(ImGuiTabBar* tab_bar)
        {
            ...
            // currently hold no discernable strings.
            char buf[256];
            char* p = buf;
