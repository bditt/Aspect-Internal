...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const float line_height = ImMax(ImMin(window->DC.CurrentLineSize.y,
                                        g.FontSize + g.Style.FramePadding.y * 2),
        g.FontSize);
    const ImRect bb(
        window->DC.CursorPos,
        window->DC.CursorPos + ImVec2(g.FontSize + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x * 2) : 0.0f), ImMax(line_height,
    ...
    if (g.NavActivateDownId == id) {
        ...
        if (nav_activated_by_code || nav_activated_by_inputs || g.ActiveId == id) {
            ...
            if ((nav_activated_by_code || nav_activated_by_inputs) && !(flags & ImGuiButtonFlags_NoNavFocus))
                SetFocusID(id, window);
            g.ActiveIdAllowNavDirFlags = (1 << ImGuiDirLeft) | (1 << ImGuiDir_Right) | (1 << ImGuiDir_Up) | (1 << ImGuiDir_Down);
        }
    }
    ...
    ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
        window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}
...
void ImGui::Separator()
{
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        (int)(flags & (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical)))); // Check that only 1
        // option is selected
    ...
    ItemSize(ImVec2(0.0f,
        0.0f)); // NB: we don't provide our width so that it doesn't
        // get feed back into AutoFit, we don't provide
        // height to not alter layout.
    if (!ItemAdd(bb, 0)) {
        if (window->DC.ColumnsSet)
            PushColumnClipRect();
        return;
    ...
    ImGuiComboFlags flags)
{
    ...
    IM_ASSERT(
        (flags & (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)); // Can't use both flags together
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive) {
            ...
            if (flags & ImGuiComboFlags_PopupAlignLeft)
                popup_window->AutoPosLastDirection = ImGuiDirLeft;
...
{
    if (fmt[0] == '%' && fmt[1] == '.' && fmt[2] == '0' && fmt[3] == 'f' && fmt[4] == 0) // Fast legacy path for "%.0f" which is expected to be the
        // most common case.
        return "%d";
...
#else
        IM_ASSERT(
            0 && "DragInt(): Invalid format string!"); // Old versions used a default
    ...
    const char* format, float power)
{
    ...
    if (power != 1.0f)
        IM_ASSERT(v_min != NULL && v_max != NULL); // When using a power curve the drag needs to
            // have known bounds
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                ...
                if (IsNavInputDown(ImGuiNavInput_TweakFast))
                    delta *= 10.0f;
                set_new_value = true;
                if ((clicked_t >= 1.0f && delta > 0.0f) || (clicked_t <= 0.0f && delta < 0.0f)) // This is to avoid applying the saturation when
    ...
    const void* v_max, const char* format, float power)
{
    ...
    if ((hovered && g.IO.MouseClicked[0]) || g.NavActivateId == id || g.NavInputId == id) {
        ...
        SetFocusID(id, window);
        FocusWindow(window);
        g.ActiveIdAllowNavDirFlags = (1 << ImGuiDirLeft) | (1 << ImGuiDir_Right);
    }
    ...
    const char** out_text_end)
{
    ...
    while (
        char c = *s++) // We are only matching for \n so we can ignore UTF-8 decoding
        if (c == '\n')
            line_count++;
    s--;
    if (s[0] != '\n' && s[0] != '\r')
...
{
    return idx > 0 ? (is_separator(obj->TextW[idx - 1]) && !is_separator(obj->TextW[idx]))
                   : 1;
}
    ...
    void* callback_user_data)
{
    ...
    IM_ASSERT(
        !((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together
        // (they both use up/down keys)
    IM_ASSERT(!(
        (flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together
    ...
    ImVec2 size = CalcItemSize(
        ...
        (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(
        frame_bb.Min,
        frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
    ...
    if (focus_requested || user_clicked || user_scrolled || user_nav_input_start) {
        if (g.ActiveId != id) {
            ...
            edit_state.InitialText.resize(
                init_buf_len + 1); // UTF-8. we use +1 to make sure that
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_osx = io.ConfigMacOSXBehaviors;
        const bool is_shortcut_key = (is_osx ? (io.KeySuper && !io.KeyCtrl)
        ...
        const bool is_startend_key_down = is_osx && io.KeySuper && !io.KeyCtrl && !io.KeyAlt; // OS X style: Line/Text Start and End using Cmd+Arrows
            // instead of Home/End
        ...
        if (apply_edit_back_to_user_buffer) {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0) {
                ...
                if (event_flag) {
                    ...
                    IM_ASSERT(
                        callback_data.Buf == edit_state.TempBuffer.Data); // Invalid to modify those fields
                    ...
                    if (callback_data.BufDirty) {
                        IM_ASSERT(
                            callback_data.BufTextLen == (int)strlen(callback_data.Buf)); // You need to maintain BufTextLen if
                            // you change the text!
                        ...
                        edit_state.CurLenA = callback_data
    ...
    if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget()) {
        if (const ImGuiPayload* payload = AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F)) {
            memcpy((float*)col, payload->Data,
                sizeof(float) * 3); // Preserve alpha if any
            value_changed = true;
        }
    ...
    ImVec2 half_sz, float bar_w)
{
    ...
    ImGui::RenderArrowPointingAt(
        ...
        ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDirLeft, IM_COL32_BLACK);
    ImGui::RenderArrowPointingAt(draw_list,
        ImVec2(pos.x + bar_w - half_sz.x, pos.y),
        half_sz, ImGuiDirLeft, IM_COL32_WHITE);
}
    ...
    ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
        ...
        ColorButton(
            "##current", col_v4,
            (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)),
    ...
    ImGuiColorEditFlags flags)
{
    ...
    ColorButton(
        ...
        (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip,
        sz);
    SameLine();
    if (flags & ImGuiColorEditFlags_NoAlpha)
        Text("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)", cr, cg, cb,
    ...
    const char* label, const char* label_end)
{
    ...
    if (!is_leaf) {
        ...
        if (g.NavId == id && g.NavMoveRequest && g.NavMoveDir == ImGuiDirLeft && is_open) {
            toggled = true;
            NavMoveRequestCancel();
        }
        if (g.NavId == id && g.NavMoveRequest && g.NavMoveDir == ImGuiDir_Right && !is_open) // If there's something upcoming on the line we may want to
...
void ImGui::TreePop()
{
    ...
    window->DC.TreeDepth--;
    if (g.NavMoveDir == ImGuiDirLeft && g.NavWindow == window && NavMoveRequestButNoResultYet())
    ...
    ImGuiTreeNodeFlags flags)
{
    ...
    bool is_open = TreeNodeBehavior(id,
        flags | ImGuiTreeNodeFlags_CollapsingHeader | (p_open ? ImGuiTreeNodeFlags_AllowItemOverlap : 0),
        label);
    if (p_open) {
        ...
        ImVec2 button_center = ImVec2(ImMin(window->DC.LastItemRect.Max.x, window->ClipRect.Max.x) - g.Style.FramePadding.x - button_radius,
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDirLeft || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu)) {
        ImGuiWindow* nav_earliest_child = g.NavWindow;
        while (
            nav_earliest_child->ParentWindow && (nav_earliest_child->ParentWindow->Flags & ImGuiWindowFlags_ChildMenu))
    ...
    PopClipRect();
    PopID();
    window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->MenuBarRect().Min.x; // Save horizontal position so next append can reuse it. This
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        pressed = Selectable(label, menu_is_open,
            ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups | (!enabled ? ImGuiSelectableFlags_Disabled : 0),
            ImVec2(w, 0.0f));
        PopStyleVar();
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added
        ...
        pressed = Selectable(label, menu_is_open,
            ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_DrawFillAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0),
            ImVec2(w, 0.0f));
        if (!enabled)
            PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
        RenderArrow(
...
void ImGui::EndMenu()
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (g.NavWindow && g.NavWindow->ParentWindow == window && g.NavMoveDir == ImGuiDirLeft && NavMoveRequestButNoResultYet() && window->DC.LayoutType == ImGuiLayoutType_Vertical) {
        ClosePopupToLevel(g.BeginPopupStack.Size, true);
        NavMoveRequestCancel();
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
        ...
        // Refresh tab width immediately if we can (for manual tab bar, WidthContent
        // will lag by one frame which is mostly noticeable when changing
        // style.FramePadding.x) Additionally, when using TabBarAddTab() to
        // manipulate tab bar order we occasionally insert new tabs that don't have
        // a width yet, and we cannot wait for the next BeginTabItem() call. We
        // cannot compute this width within TabBarAddTab() because font size depends
    ...
    if (width_excess > 0.0f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown)) {
        ...
        while (width_excess > 0.0f && tab_count_same_width < tab_bar->Tabs.Size) {
            ...
            float width_to_remove_per_tab = ImMin(width_excess / tab_count_same_width,
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    if (ArrowButtonEx("##<", ImGuiDirLeft, arrow_button_size,
    ...
    ImGuiTabItemFlags flags)
{
    ...
    if (pressed || ((flags & ImGuiTabItemFlags_SetSelected) && !tab_contents_visible)) // SetSelected can only be passed on
        // explicit tab bar
    ...
    ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    draw_list->AddConvexPolyFilled(draw_list->_Path.Data, draw_list->_Path.Size,
        col);
    if (g.Style.TabBorderSize > 0.0f)
        draw_list->AddPolyline(draw_list->_Path.Data, draw_list->_Path.Size,
            GetColorU32(ImGuiCol_Border), false,
    ...
    ImGuiID close_button_id)
{
    ...
    if (flags & ImGuiTabItemFlags_UnsavedDocument) {
        ...
        ImVec2 unsaved_marker_pos(
            ImMin(bb.Min.x + style.FramePadding.x + label_size.x + 2,
        ...
        RenderTextClippedEx(draw_list, unsaved_marker_pos,
            bb.Max - style.FramePadding, TAB_UNSAVED_MARKER, NULL,
            NULL);
    }
    ...
    if (label_size.x > text_ellipsis_clip_bb.GetWidth()) {
        ...
        float label_size_clipped_x = g.Font
                                         ->CalcTextSizeA(
                                             g.FontSize,
                                             text_ellipsis_clip_bb.GetWidth() - ellipsis_width + 1.0f, 0.0f,
                                             label, label_display_end, &label_end)
                                         .x;
        if (label_end == label && label_end < label_display_end) // Always display at least 1 character if
