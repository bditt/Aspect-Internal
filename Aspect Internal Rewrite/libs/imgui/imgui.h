...
IMGUI_API void
NewFrame(); // start a new ImGui frame, you can submit any
...
IMGUI_API void ShowMetricsWindow(
    bool* p_open = NULL); // create metrics window. display Dear ImGui internals: draw
...
IMGUI_API void SetWindowFontScale(
    float scale); // set font scale. Adjust IO.FontGlobalScale if
...
IMGUI_API void SetWindowFocus(
    const char* name); // set named window to be focused /
...
IMGUI_API void SameLine(
    ...
    float spacing_w = -1.0f); // call between widgets or groups to layout them
...
IMGUI_API void Dummy(
    const ImVec2& size); // add a dummy item of given size. unlike InvisibleButton(),
...
IMGUI_API void SetCursorScreenPos(
    const ImVec2& pos); // cursor position in absolute screen
    // coordinates [0..io.DisplaySize]
IMGUI_API void
...
IMGUI_API bool IsPopupOpen(
    const char* str_id); // return true if the popup is open at the
...
IMGUI_API ImVec2 GetMouseDragDelta(
    int button = 0,
    float lock_threshold = -1.0f); // return the delta from the initial clicking position. This is
...
IMGUI_API void CaptureMouseFromApp(
    bool want_capture_mouse_value = true); // attention: misleading name! manually override
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_NoFocusOnAppearing = 1 << 12, // Disable taking focus when transitioning from hidden to
    // visible state
    ...
    ImGuiWindowFlags_UnsavedDocument = 1 << 20, // Append '*' to title without affecting the ID, as a
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_CallbackAlways = 1 << 8, // Callback on each iteration. User code may query cursor
    // position, modify text buffer.
    ImGuiInputTextFlags_CallbackCharFilter = 1 << 9, // Callback on character inputs to replace or discard them.
    ...
    ImGuiInputTextFlags_CallbackResize = 1 << 18, // Callback on buffer capacity changes request (beyond
...
enum ImGuiComboFlags_ {
    ...
    ImGuiComboFlags_HeightSmall = 1 << 1, // Max ~4 items visible. Tip: If you want your combo popup to be
...
enum ImGuiTabBarFlags_ {
    ...
    ImGuiTabBarFlags_Reorderable = 1 << 0, // Allow manually dragging tabs to re-order them + New tabs are
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_UnsavedDocument = 1 << 0, // Append '*' to title without affecting the ID, as a convenience
    ...
    ImGuiTabItemFlags_SetSelected = 1 << 1, // Trigger flag to programatically make the tab selected when
...
enum ImGuiFocusedFlags_ {
    ...
    ImGuiFocusedFlags_ChildWindows = 1 << 0, // IsWindowFocused(): Return true if any children of the window
    // is focused
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceNoDisableHover = 1 << 1, // By default, when dragging we clear data so that
    ...
    ImGuiDragDropFlags_AcceptPeekOnly = ImGuiDragDropFlags_AcceptBeforeDelivery | ImGuiDragDropFlags_AcceptNoDrawDefaultRect // For peeking ahead and
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseCursors = 1 << 1, // Back-end supports honoring GetMouseCursor() value to change
    // the OS cursor shape.
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags__OptionsDefault = ImGuiColorEditFlags_Uint8 | ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_PickerHueBar // Change application default using
    // SetColorEditOptions()
};
...
struct ImGuiIO {
    ...
    ImVec2 DisplaySize; // <unset>          // Main display size, in pixels. For
    ...
    ImVec2 DisplayVisibleMin; // <unset>          // [OBSOLETE] If you use
        // DisplaySize as a virtual space larger than your
        // screen, set DisplayVisibleMin/Max to the visible
        // area.
    ImVec2 DisplayVisibleMax; // <unset>          // [OBSOLETE] Just use
...
enum ImDrawListFlags_ {
    ...
    ImDrawListFlags_AntiAliasedLines = 1 << 0, // Lines are anti-aliased (*2 the number of triangles for 1.0f
...
struct ImDrawList {
    ...
    IMGUI_API void PushClipRect(
        ...
        bool intersect_with_current_clip_rect = false); // Render-level scissoring. This is passed down to your
    ...
    IMGUI_API void AddRectFilledMultiColor(const ImVec2& a, const ImVec2& b,
        ImU32 col_uprLeft, ImU32 col_upr_right,
        ImU32 col_bot_right,
        ImU32 col_bot_left);
...
struct ImDrawData {
    ...
    IMGUI_API void ScaleClipRects(
        const ImVec2& sc); // Helper to scale the ClipRect field of each ImDrawCmd. Use if
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFont* AddFontFromMemoryCompressedTTF(
        ...
        const ImWchar* glyph_ranges = NULL); // 'compressed_font_data' still owned by caller. Compress with
    ...
    struct CustomRect {
        unsigned int
            ID; // Input    // User ID. Use <0x10000 to map into a font
