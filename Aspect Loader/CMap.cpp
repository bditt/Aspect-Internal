...
namespace ManualMap
{
	...
	byte ShellCode64[] =
	{
		0x9C, 0x50, 0x53, 0x51, 0x52, 0x41, 0x50, 0x41, 0x51, 0x41, 0x52, 0x41, 0x53, 0x48, 0x83, 0xEC, 0x28, 0x48, 0xB9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xD0, 0x48, 0x83, 0xC4, 0x28, 0x41, 0x5B, 0x41, 0x5A, 0x41, 0x59, 0x41, 0x58, 0x5A, 0x59, 0x5B, 0x58, 0x9D, 0x49, 0xB8, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0x48, 0x83, 0xC4, 0x08, 0x41, 0xFF, 0xE0
	};
	byte ShellCode32[] =
	{
		0x60, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x5B, 0x81, 0xEB, 0x06, 0x00, 0x00, 0x00, 0xB8, 0xCC, 0xCC, 0xCC, 0xCC, 0xBA, 0xCC, 0xCC, 0xCC, 0xCC, 0x90, 0x52, 0xFF, 0xD0, 0x61, 0x68, 0xCC, 0xCC, 0xCC, 0xCC, 0xC3
	};
	...
	BOOL LoadFileFromResourcesA(HANDLE hProcess, HMODULE hDll, LPCSTR lpName, LPCSTR ResourceType, DWORD Flags, WDLL*& wFile)
	{
		//START
		...
		Result = LoadFileFromResourcesW(hProcess, hDll, wName, wResType, Flags, wFile);
		if (wName) delete wName;
		if (wResType) delete wResType;
		//END
		return Result;
	...
	BOOL LoadFileFromResourcesW(HANDLE hProcess, HMODULE hDll, LPCWSTR lpName, LPCWSTR ResourceType, DWORD Flags, WDLL*& wFile)
	{
		...
		hResource = FindResourceW(hDll, lpName, ResourceType);
		if (!hResource)
		{
			SetLastError(1891);
			return 0;
		...
		wFile->Module = reinterpret_cast<HMODULE>(LoadResource(hDll, hResource));
	...
	BOOL LoadFileFromMemory(HANDLE hProcess, LPVOID lpDll, DWORD Flags, WDLL*& wFile)
	{
		//VM_START
		PIMAGE_SECTION_HEADER SectionHeaders = 0;
		LPVOID lpwDll = 0, lpwThread = 0, lpShellCode = 0;
		HANDLE hThread = 0;
		BOOL Status = TRUE;
		...
		_RtlCreateUserThread RtlCreateUserThread = reinterpret_cast<_RtlCreateUserThread>(GetProcAddress(GetModuleHandleA("ntdll.dll"), "RtlCreateUserThread"));
		if (!wFile)
		{
			SetLastError(1890);
			return 0;
		...
		if (wFile->IsNtDll)
			Functions[6] = reinterpret_cast<DWORD_PTR>(GetModuleHandleA("ntdll.dll"));
		...
		for (int i = 0; i < wFile->NtHeader->FileHeader.NumberOfSections; i++)
		{
			...
			{
				memcpy(reinterpret_cast<PVOID>(reinterpret_cast<DWORD_PTR>(wFile->Image) + SectionHeaders[i].VirtualAddress), reinterpret_cast<PVOID>(reinterpret_cast<DWORD_PTR>(wFile->Module) + SectionHeaders[i].PointerToRawData), SectionHeaders[i].SizeOfRawData);
		...
		if (hProcess != GetCurrentProcess())
		{
			lpwDll = VirtualAllocEx(wFile->hProcess, 0, 0x2000, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
			if (!lpwDll)
			{
				VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
				VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
			...
			if (!WriteProcessMemory(wFile->hProcess, lpwDll, wFile, 0x2000, &Written))
			{
				VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
				VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
			...
			lpwThread = VirtualAllocEx(wFile->hProcess, 0, 0x4000, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
			if (!lpwThread)
			{
				VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
				VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
			...
			if (!WriteProcessMemory(wFile->hProcess, lpwThread, BuildFile_Thread, 0x4000, &Written))
			{
				VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
				VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
				VirtualFreeEx(wFile->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
			...
			if ((wFile->Flags & ManualMap_FLAGS::HIJACK_THREAD))
			{
				lpShellCode = VirtualAllocEx(wFile->hProcess, 0, 0x1000, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
				if (!lpShellCode)
				{
					VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
				...
				SuspendThread(wFile->hThread);
				if (!GetThreadContext(wFile->hThread, &ctx))
				{
					VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, lpwThread, 0x1000, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, lpShellCode, 0x1000, MEM_DECOMMIT);
					ResumeThread(hThread);
					SetLastError(2035);
					return 0;
				}
#if defined _M_X64
				* reinterpret_cast<DWORD_PTR*>(ShellCode64 + 29) = reinterpret_cast<DWORD_PTR>(lpwThread);
				*reinterpret_cast<DWORD_PTR*>(ShellCode64 + 19) = reinterpret_cast<DWORD_PTR>(lpwDll);
				*reinterpret_cast<DWORD_PTR*>(ShellCode64 + 58) = ctx.Rip;
				if (!WriteProcessMemory(wFile->hProcess, lpShellCode, ShellCode64, _countof(ShellCode64), 0))
				{
					VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
				...
				ctx.Rsp -= 8;
				ctx.Rip = reinterpret_cast<DWORD_PTR>(lpShellCode);
#elif defined _M_IX86 
				* reinterpret_cast<DWORD*>(&ShellCode32[29]) = (ctx.Eip);
				*reinterpret_cast<DWORD*>(&ShellCode32[14]) = reinterpret_cast<DWORD>(lpwThread);
				*reinterpret_cast<DWORD*>(&ShellCode32[19]) = reinterpret_cast<DWORD>(lpwDll);
				if (!WriteProcessMemory(wFile->hProcess, lpShellCode, ShellCode32, _countof(ShellCode32), 0))
				{
					VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
				...
				ctx.Eip = reinterpret_cast<DWORD>(lpShellCode);
#endif
				if (!SetThreadContext(wFile->hThread, &ctx))
				{
					VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT);
					VirtualFreeEx(wFile->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
				...
				ResumeThread(wFile->hThread);
			}
			else
			{
				if ((RtlCreateUserThread(wFile->hProcess, 0, 0, 0, 0, 0, reinterpret_cast<PVOID>(lpwThread), lpwDll, &hThread, 0)))
				{
					std::cout << "MEM_DECOMMIT" << std::endl;
					if (!VirtualFreeEx(wFile->hProcess, wFile->Image, wFile->ImageSize, MEM_DECOMMIT))
						std::cout << "VirtualFreeEx Image MEM_DECOMMIT" << std::endl;
					if (!VirtualFreeEx(wFile->hProcess, wFile->Parameter, sizeof(DWORD_PTR) * _countof(Functions), MEM_DECOMMIT))
						std::cout << "VirtualFreeEx Parameter MEM_DECOMMIT" << std::endl;
					if (!VirtualFreeEx(wFile->hProcess, lpwDll, 0x1000, MEM_DECOMMIT))
						std::cout << "VirtualFreeEx lpwDll MEM_DECOMMIT" << std::endl;
					if (!VirtualFreeEx(wFile->hProcess, lpwThread, 0x1000, MEM_DECOMMIT))
						std::cout << "VirtualFreeEx Thread lpwThread" << std::endl;
					std::cout << "Done!" << std::endl;
					SetLastError(2008);
					return 0;
				}
				if ((wFile->Flags & ManualMap_FLAGS::WAIT_FOR_THREAD))
				{
	...
	BOOL UnloadFile(WDLL* wDll)
	{
		...
		BOOL Result = TRUE;
		LPVOID lpUnloadThread = 0, lpwDll = 0, lpShellCode = 0;
		HANDLE hThread = 0;
		CONTEXT ctx;
		ZeroMemory(&ctx, sizeof(ctx));
		...
		if (!(wDll->Flags & ManualMap_FLAGS::CALL_NO_ENTRYPOINT))
		{
			if (wDll->Entrypoint)
			{
				...
				if ((wDll->Flags & ManualMap_FLAGS::HIJACK_THREAD))
				{
					lpShellCode = VirtualAllocEx(wDll->hProcess, 0, 0x1000, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
					if (!lpShellCode)
					{
					...
					ctx.ContextFlags = CONTEXT_FULL;
					SuspendThread(wDll->hThread);
					if (!GetThreadContext(wDll->hThread, &ctx))
					{
						VirtualFreeEx(wDll->hProcess, lpUnloadThread, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpShellCode, 0x1000, MEM_DECOMMIT);
						ResumeThread(wDll->hThread);
						SetLastError(2035);
						return 0;
					}
#if defined _M_X64
					* reinterpret_cast<DWORD_PTR*>(ShellCode64 + 29) = reinterpret_cast<DWORD_PTR>(lpUnloadThread);
					*reinterpret_cast<DWORD_PTR*>(ShellCode64 + 19) = reinterpret_cast<DWORD_PTR>(lpwDll);
					*reinterpret_cast<DWORD_PTR*>(ShellCode64 + 58) = ctx.Rip;
					if (!WriteProcessMemory(wDll->hProcess, lpShellCode, ShellCode64, _countof(ShellCode64), 0))
					{
						VirtualFreeEx(wDll->hProcess, lpUnloadThread, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpShellCode, 0x1000, MEM_DECOMMIT);
...
#elif defined _M_IX86 
					* reinterpret_cast<DWORD*>(&ShellCode32[29]) = (ctx.Eip);
					*reinterpret_cast<DWORD*>(&ShellCode32[14]) = reinterpret_cast<DWORD>(lpUnloadThread);
					*reinterpret_cast<DWORD*>(&ShellCode32[19]) = reinterpret_cast<DWORD>(lpwDll);
					if (!WriteProcessMemory(wDll->hProcess, lpShellCode, ShellCode32, _countof(ShellCode32), 0))
					{
						VirtualFreeEx(wDll->hProcess, lpUnloadThread, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpShellCode, 0x1000, MEM_DECOMMIT);
					...
					ctx.Eip = reinterpret_cast<DWORD>(lpShellCode);
#endif
					if (!SetThreadContext(wDll->hThread, &ctx))
					{
						VirtualFreeEx(wDll->hProcess, lpUnloadThread, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpwDll, 0x1000, MEM_DECOMMIT);
						VirtualFreeEx(wDll->hProcess, lpShellCode, 0x1000, MEM_DECOMMIT);
				...
				else
				{
					...
					if ((wDll->Flags & ManualMap_FLAGS::WAIT_FOR_THREAD))
						WaitForSingleObject(hThread, INFINITE);
					if (lpUnloadThread &&
						!VirtualFreeEx(wDll->hProcess, lpUnloadThread, 0x1000, MEM_DECOMMIT) ||
						lpwDll &&
	...
	BOOL WINAPI BuildFile_Thread(WDLL* wDll)
	{
		...
		if ((wDll->Flags & ManualMap_FLAGS::ERASE_PE))
		{
			for (int i = 0; i < 0x1000; i++)
				reinterpret_cast<BYTE*>(wDll->Image)[i] = 0;
		}
		return Status;
	};
	DWORD WINAPI UnloadFile_Thread(WDLL* wDll)
